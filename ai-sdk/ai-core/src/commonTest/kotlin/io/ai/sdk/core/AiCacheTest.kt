...
class AiCacheTest {
    ...
    private lateinit var cache: AiCache
    ...
    fun setup() {
        ...
        cache = AiCache(cacheDirPath, fileSystem = fs)
    }
    ...
    fun trimIfNeededEvictsOldestFilesWhenOverLimit() {
        val smallCache = AiCache(cacheDirPath, maxSizeBytes = 10, fileSystem = fs)
    ...
    fun customStrategyIsCalledOnCommit() {
        ...
        val customStrategy = CacheStrategy { files, _, _ ->
            files.also { evicted.addAll(it) }
        }
        val customCache = AiCache(cacheDirPath, maxSizeBytes = 10, strategy = customStrategy, fileSystem = fs)
