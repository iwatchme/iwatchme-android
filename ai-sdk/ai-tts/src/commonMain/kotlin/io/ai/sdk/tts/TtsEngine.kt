...
class TtsEngine private constructor(
    ...
    fun preview(text: String, source: Int, voice: TtsVoiceParams): Flow<TtsEvent> = flow {
        ...
        try {
            ...
            throw e
        } catch (e: io.ai.sdk.internal.AiSdkException) {
    ...
    class Builder {
        ...
        fun cacheStrategy(strategy: CacheStrategy): Builder {
        ...
        fun build(): TtsEngine {
            ...
            val pools = sdkRegistrations.map { (source, reg) ->
                source to SdkPool(
                    factory = reg.factory,
                    maxSize = reg.poolSize,
                    initializer = { it.initialize() },
                    releaser = { it.release() },
                )
            }.toMap()
            val strategy = cacheStrategy ?: LruCacheStrategy()
            val cache = AiCache(dirPath, strategy = strategy)
