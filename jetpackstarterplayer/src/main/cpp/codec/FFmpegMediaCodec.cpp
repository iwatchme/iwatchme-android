...
int FFmpegMediaCodec::decode_write(AVCodecContext *avctx, AVPacket *packet) {
    ...
    while (1) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
            LOGE("avcodec_receive_frame EAGAIN or EOF");
            av_frame_free(&frame);
...
int FFmpegMediaCodec::decode(const std::string &input, const std::string &output) {
    ...
    /* open the input file */
    int result = avformat_open_input(&input_ctx, input.c_str(), NULL, NULL);
    char *error = new char[1024];
     av_strerror(result, error, 1024);
     LOGE("avformat_open_input error %s", error);
    if (result != 0) {
    ...
    remove(output.c_str());
    LOGE("remove output_file %s", output.c_str());
    /* open the file to dump raw data */
    output_file = fopen(output.c_str(), "w+");
    LOGE("open output_file %d", output_file == NULL? 0: 1);

    int error_code = errno;
    if (output_file == NULL) {
        LOGE("Failed to open output file %s, reason: %s", output.c_str(), strerror(error_code));
        return -1;
    }
